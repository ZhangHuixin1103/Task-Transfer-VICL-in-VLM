...
from efficiency.base import (
    EfficiencyAdapter,
    InferenceResult,
    load_dataset_records,
    load_vicl_sample,
)
from efficiency.benchmark import _external_repository, parser, run_benchmark
from efficiency.prompting import RELATION_REQUEST
from efficiency.suite import parser as suite_parser, run_suite


TASK_TRANSFER = Path(__file__).resolve().parents[2]
EXTERNAL_ROOT_VALUE = os.environ.get("VICL_EXTERNAL_ROOT")
EXTERNAL_ROOT = (
    Path(EXTERNAL_ROOT_VALUE).expanduser().resolve() if EXTERNAL_ROOT_VALUE else None
)
...
class InterfaceContractTest(unittest.TestCase):
    ...
    def test_t2t_adapter_caches_json_and_selects_task_subset_in_memory(self):
        from efficiency.adapters.t2t import T2TVICLAdapter
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch(
                "efficiency.adapters.t2t.load_dataset_records",
    ...
    def test_t2t_efficiency_options_preserve_original_defaults(self):
        ...
        self.assert_function_defaults(
            ...
            "generate_image_omnigen",
            {
                ...
                # efficiency adapter supplies its seed explicitly.
                "seed": None,
    ...
    def test_external_repository_requires_an_explicit_root(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesRegex(ValueError, "VICL_EXTERNAL_ROOT"):
    ...
    def test_parallel_launcher_assigns_one_visible_gpu(self):
        ...
        from efficiency.launch_t2t import Job, _child_environment
    ...
    def test_parallel_launcher_runs_jobs_with_isolated_gpu_visibility(self):
        from efficiency.launch_t2t import main as launch_main
    ...
    @unittest.skipUnless(
        EXTERNAL_ROOT is not None and EXTERNAL_ROOT.is_dir(),
        "set VICL_EXTERNAL_ROOT to audit external source entry points",
    )
    ...
    def test_condition_resources_are_isolated(self):
        ...
        with mock.patch("efficiency.benchmark.build_adapter", return_value=adapter):
    ...
    def test_multi_task_suite_uses_distinct_queries_and_builds_macro_row(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch("efficiency.suite.build_adapter", return_value=adapter):
    ...
    def test_suite_reuses_warmup_only_when_every_query_is_measured(self):
        from efficiency.suite import _select_warmup_indices
