...
from comparison.adapters.toy import ToyAdapter
...
from comparison.report import flatten_result, write_reports
from comparison.suite import _aggregate_flop_profiles, _select_flop_indices
...
class MetricsTest(unittest.TestCase):
    ...
    def test_module_hook_fallback_counts_dynamic_model_execution(self):
        adapter = ToyAdapter(device="cpu")
        adapter.setup()
        with mock.patch("comparison.flops.AuditedFlopCounterMode", None):
            flops = profile_flops(
    ...
    def test_dispatch_failure_uses_partial_module_hook_fallback(self):
        ...
        adapter.setup()
        with mock.patch(
            "comparison.metrics.dispatch_flop_count",
