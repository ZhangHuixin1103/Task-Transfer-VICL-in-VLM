...
class TaskSpec:
    ...
    source_task_a: str | None = None
    source_task_b: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if (self.source_task_a is None) != (self.source_task_b is None):
            raise ValueError(
                f"Task {self.name!r} must define source_task_a and "
                "source_task_b together"
            )
        if self.source_task_a is not None and self.task_a is None:
            raise ValueError(
                f"Task {self.name!r} can only use source-task filters with the "
                "embedded taskA/taskB JSON schema"
            )
        if not self.uses_embedded_vicl and (not self.demo_input or not self.demo_output):
            raise ValueError(
                f"Same-task entry {self.name!r} requires demo_input and demo_output"
            )

    @property
    def uses_embedded_vicl(self) -> bool:
        return self.task_a is not None

    @property
    def is_cross_task(self) -> bool:
        return self.uses_embedded_vicl and self.task_a != self.task_b

    @property
    def is_same_task_vicl(self) -> bool:
        return self.uses_embedded_vicl and self.task_a == self.task_b
...
def matching_record_indices(
    ...
    """Return source indices for one embedded VICL group, or None for paired JSON."""
    if not task.uses_embedded_vicl:
        return None
    ...
    for index, record in enumerate(records):
        ...
        task_a_matches = task.task_a == "*" or record_task_a == task.task_a
        if not task_a_matches or record_task_b != task.task_b:
            continue
        if task.source_task_a is not None:
            source_task_a = record.get("source_task_a")
            source_task_b = record.get("source_task_b")
            if source_task_a is None or source_task_b is None:
                raise KeyError(
                    f"Embedded VICL record {index} is missing source_task_a or "
                    "source_task_b required by the manifest"
                )
            if (
                source_task_a != task.source_task_a
                or source_task_b != task.source_task_b
            ):
                continue
        output_task_a = _path_task_name(record["taskA_output"])
        output_task_b = _path_task_name(record["taskB_output"])
        if output_task_a != record_task_a or output_task_b != record_task_b:
            raise ValueError(
    ...
    if not selected:
        raise ValueError(
            f"No records match embedded pair {task.task_a} -> {task.task_b} "
            f"with source filter {task.source_task_a} -> {task.source_task_b} "
...
def validate_task(
    ...
    result["native_pair_size_policy"] = (
        "Recorded as a warning, not a failure: controlled comparison adapters "
...
def validate_json_file(
    ...
    result["native_pair_size_policy"] = (
        "Recorded as a warning, not a failure under the fixed-size comparison protocol."
    )
    result["status"] = "ok" if failures == 0 else "incomplete"
    return result
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--manifest",
        type=Path,
        default=Path(__file__).with_name("competitor_tasks.json"),
    )
